...
<table width="100%" border="0" cellspacing="0" cellpadding="0" style="margin-top:10px;">
	...
	<c:forEach var="purchase" items="${list}">
		...
		<tr class="ct_list_pop">
			...
			<c:if test="${fn:trim(purchase.tranCode)=='0'}"> 현재 구매전 입니다. </c:if>
			<c:if test="${fn:trim(purchase.tranCode)=='1'}"> 현재 구매완료 입니다. </c:if>
			<c:if test="${fn:trim(purchase.tranCode)=='2'}"> 현재 배송 중 입니다. </c:if>
			<c:if test="${fn:trim(purchase.tranCode)=='3'}"> 현재 배송완료 입니다. </c:if>
		</td>
		<td></td>
		<td align="left">
			<c:if test="${fn:trim(purchase.tranCode)=='3'}">
			<a href="/updateTranCode.do?prod_no=${purchases.purchaseProd.prodNo}&tran_code=${purchase.tranCode}">물건도착</a></c:if>
